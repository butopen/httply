...
export const notificationStore = loggedWritable<Notifications>({message: "", show: false, valideUpTo: 0})


export function updateNotification(message, timeMs = 4000){
    notificationStore.update({message, show: true, valideUpTo: new Date().getTime() + timeMs})
}
